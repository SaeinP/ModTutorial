...
public class MemoiteAxeProjectileEntity extends AbstractArrow {
    private float rotation;
    public Vec2 groundedOffset;
    public int dmg;

    public MemoiteAxeProjectileEntity(EntityType<? extends AbstractArrow> pEntityType, Level pLevel, int dmg) {
        super(pEntityType, pLevel);
        this.dmg = dmg;
    }

    public MemoiteAxeProjectileEntity(LivingEntity shooter, Level level) {
        super(ModEntities.MEMOITE_AXE.get(), shooter, level, new ItemStack(ModItems.MEMOITE_AXE.get()), null);
    }

    @Override
    protected ItemStack getDefaultPickupItem() {
        return new ItemStack(ModItems.MEMOITE_AXE.get()).hurtAndBreak(dmg);;
    }
